...
* different VM's in what gets sent as a <em>key typed</em> * event, and this action tries to filter out the undesired
...
* entry is done through the <code>replaceSelection</code> * method on the target text component. This is the
...
static class DefaultKeyTypedAction extends TextAction {
	...
	*/
	...
	if ((target != null) && (e != null)) {
		...
		if ((content != null) && (content.length() > 0) &&		 (((mod & ActionEvent.ALT_MASK) == 0) ||
...
static {
	Keymap map = JTextComponent.getKeymap(JTextComponent.DEFAULT_KEYMAP);		if (map == null)
	System.out.println("Default keymap doesn't exist, AltGr patch not applied");
	else {
	map.setDefaultAction(new DefaultKeyTypedAction());
	//System.out.println("AltGr patch applied");
	}
}
